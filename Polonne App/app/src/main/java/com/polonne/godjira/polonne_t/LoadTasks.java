...
public class LoadTasks {
    ...
    private class preLoad extends AsyncTask<Void, Void, Boolean> {
        ...
        protected Boolean doInBackground(Void... params) {
            PreLoadFile preLoad = new PreLoadFile();
            ...
            try {
                ...
                JSONArray arjMapPoints = jsonResponse.getJSONArray("mapPoints");
                for (int i = 0; arjMapPoints.length() > i; i++) {
                    ContentValues cv = new ContentValues();

                    JSONObject mapPointsJsonObj = (JSONObject) arjMapPoints.get(i);
                    cv.put("id", mapPointsJsonObj.getInt("id"));
                    cv.put("title", mapPointsJsonObj.getString("title"));
                    cv.put("latitude", mapPointsJsonObj.getDouble("latitude"));
                    cv.put("longitude", mapPointsJsonObj.getDouble("longitude"));
                    db.insert("mapPoints", null, cv);
                }
                return true;
